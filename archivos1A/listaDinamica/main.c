...
int main()
{
    ...
    if(guardarEmpleadosCSV(lista2, tam2, "lista2.csv")){

        printf("empleados guardados exitosamente en .csv\n\n");

    }else{

    printf("no se pudo guardar empleados en .csv\n\n");
...
int guardarEmpleadosCSV(eEmpleado** lista, int tam, char* path){
    ...
    for(int i=0; i<tam; i++){
        ...
        auxpEmp=*(lista+i);
        ok = fprintf(f, "%d,%s,%.2f\n", auxpEmp->id, auxpEmp->nombre, auxpEmp->sueldo);
